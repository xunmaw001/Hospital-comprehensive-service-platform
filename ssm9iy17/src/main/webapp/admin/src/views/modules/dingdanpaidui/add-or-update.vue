<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.chufangtupian" label="处方图片" prop="chufangtupian">
          <file-upload
          tip="点击上传处方图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.chufangtupian" label="处方图片" prop="chufangtupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.chufangtupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="订单号码" prop="dingdanhaoma">
          <el-input v-model="ruleForm.dingdanhaoma" 
              placeholder="订单号码" clearable  :readonly="ro.dingdanhaoma"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="订单号码" prop="dingdanhaoma">
              <el-input v-model="ruleForm.dingdanhaoma" 
                placeholder="订单号码" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="当前号码" prop="dangqianhaoma">
          <el-input v-model="ruleForm.dangqianhaoma" 
              placeholder="当前号码" clearable  :readonly="ro.dangqianhaoma"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="当前号码" prop="dangqianhaoma">
              <el-input v-model="ruleForm.dangqianhaoma" 
                placeholder="当前号码" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="推送时间" prop="tuisongshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.tuisongshijian" 
                type="datetime"
                :readonly="ro.tuisongshijian"
                placeholder="推送时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tuisongshijian" label="推送时间" prop="tuisongshijian">
              <el-input v-model="ruleForm.tuisongshijian" 
                placeholder="推送时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.tuisongshijian = this.getCurDateTime()
  ...
  methods: {
	...
	if(this.ruleForm.chufangtupian!=null) {
		this.ruleForm.chufangtupian = this.ruleForm.chufangtupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dingdanpaidui/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dingdanpaiduiCrossAddOrUpdateFlag = false;
						 this.parent.search();
	...
	this.ruleForm.chufangtupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
